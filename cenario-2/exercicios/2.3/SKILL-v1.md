...
description: >-
  ...
  de input E output, logging com pino, erros padronizados e campo source_document
...
**Objetivo:** padronizar a criação de endpoints HTTP em **Azure Functions v4 + TypeScript strict** no projeto NovaTech Assistant, de modo que qualquer endpoint gerado por agente (Copilot, Claude Code) saia consistente com o `AGENTS.md`: thin handler, validação Zod de **input e output**, logging estruturado com `pino`, erros padronizados via `AppError`, e campo `source_document` presente em toda resposta de domínio.
...
- Testes do endpoint → `skills/domain/testing-patterns.md` / `skills/artifact/create-integration-test.md`.
...
- **DEVE** criar o endpoint em `src/functions/<nome>/` com, no mínimo:
  ...
  - `response-builder.ts` — serialização da resposta (sucesso e erro) em `HttpResponseInit`.
...
- **DEVE** rodar em `strict: true`. **NÃO DEVE** usar `any` (use `unknown` + narrowing).
...
- **DEVE** validar o objeto de resposta com Zod **antes** de retornar; falha de output é `InternalServerError`, não erro do cliente.
- **DEVE** usar `.strict()` nos schemas (rejeitar campos desconhecidos).
...
### 4.4 `source_document` (obrigatório — guardrail de produto)
- **DEVE** incluir o campo `source_document` no JSON de toda resposta de domínio, **inclusive em baixa confiança**.
...
- **NÃO DEVE** retornar resposta de domínio sem `source_document`, nem em fallback.
...
- **DEVE** logar erro com nível `error` passando o objeto de erro: `logger.error({ err }, "mensagem")`.
...
1. **Ler dependências** (§3): `AGENTS.md` + 3 skills Foundation.
2. **Criar a pasta** `src/functions/<nome>/`.
3. **`validator.ts`** — definir `<nome>RequestSchema` e `<nome>ResponseSchema` (Zod, `.strict()`, budget enforçado). Reexportar tipos de `src/shared/types.ts`.
4. **`response-builder.ts`** — `toSuccessResponse(...)` e `toErrorResponse(error: AppError)` retornando `HttpResponseInit` com `content-type: application/json; charset=utf-8`, validando o corpo com o schema.
5. **`handler.ts`** —
   ...
   4. Orquestrar `src/services/` (I/O, prompt, completion, seleção de fonte).
   5. Montar a resposta **incluindo `source_document`**.
   6. `schema.safeParse` do output → `InternalServerError` se falhar.
   7. `return toSuccessResponse(...)`.
   8. `catch (error: unknown)` → `mapUnknownError` → `logger.error({ err })` → `return toErrorResponse(appError)`.
   9. `app.http(...)`.
6. **Verificar** com o checklist (§8) e os critérios automatizáveis (§9): `npm run lint && npm run build && npm run test`.

---

## 6. Exemplos DO / DON'T (TypeScript real)

### 6.1 DO — `validator.ts` (input + output, Zod `.strict()`, budget enforçado)
```typescript
import { z } from "zod";
...
### 6.2 DO — `response-builder.ts` (serialização pura, valida o corpo)
```typescript
...
### 6.3 DO — `handler.ts` (thin, valida I/O, loga, erro padronizado, `source_document`)
```typescript
...
export async function queryHandler(
  ...
  try {
    ...
    return toSuccessResponse(validated.data);
  } catch (error: unknown) {
...
export async function queryHandler(request: any, context: any) {
  ...
  // ❌ sem try/catch: exceção vaza como 500 cru, sem corpo padronizado
  // ❌ sem validação Zod do input
  ...
  // ❌ resposta SEM source_document (viola guardrail) e SEM validação de output
  return { status: 200, jsonBody: { answer: chunks[0].text } };
}

// ❌ rota/método implícitos; sem authLevel consciente
app.http("query", { handler: queryHandler });
```
**Correção:** aplicar a estrutura de 6.1–6.3: `any` → tipos do Azure + `unknown`; `console.log` → `logger`; `fetch` inline → service em `src/services/`; adicionar Zod input/output; incluir `source_document`; `try/catch` + `mapUnknownError` + `toErrorResponse`; declarar `methods`, `route`, `authLevel`.
...
| A3 | Sem validação de **output** | Valida input mas retorna objeto cru | `responseSchema.safeParse(response)` → `InternalServerError` em falha. |
...
| A11 | Schema sem `.strict()` | Aceita campos desconhecidos | Adicionar `.strict()` a todos os objetos Zod. |
...
- [ ] Nenhum `any`; nenhum `@ts-ignore`/`@ts-expect-error` sem justificativa.
- [ ] Imports relativos com extensão `.js`; `import type` para tipos; sem `require`/`export default`.
- [ ] Zod valida **input** (`safeParse` → `ValidationError`) e **output** (`safeParse` → `InternalServerError`); todos os objetos com `.strict()`.
...
- [ ] `logger` (pino) usado; zero `console.*`; request logado com `invocationId/method/url`; erro logado com `{ err }`.
...
|---|---|---|---|
| V1 | Sem `any` | `tsc --noEmit` em `strict` + lint `@typescript-eslint/no-explicit-any` | Bloqueia merge |
...
| V4 | Imports relativos com `.js` | `grep -rnE "from \"\.\.?/.*[^.][^j][^s]\"" ...` sem ocorrências relativas sem `.js` | Bloqueia merge |
| V5 | Output valida `source_document` | grep do `responseSchema` contém `source_document`; teste de happy path afirma `body.source_document` definido | Bloqueia merge |
| V6 | Input e output usam Zod `safeParse` | grep por `safeParse` em handler ≥ 2 ocorrências (input + output) | Review |
| V7 | Erro padronizado | teste de input inválido retorna `status 400` e corpo com `error` e `code` | Bloqueia merge |
| V8 | Budget enforçado | schema rejeita `top_k: 6` e `conversation_history` com 4 turnos (teste unitário) | Review |
| V9 | `app.http` completo | grep do bloco `app.http` contém `methods`, `route`, `authLevel` | Review |
| V10 | Cobertura | `vitest run --coverage` ≥ 80% linhas (`vitest.config.ts`) | Bloqueia merge |

> Sugestão de gate: V1–V5, V7 e V10 são **bloqueantes** (CI vermelho impede merge); V6, V8, V9 entram no code review do Tech Lead.

---

## 10. Regra → Evidência esperada no código

Mapa direto entre cada regra prescritiva e o **trecho concreto** que um revisor (humano ou agente) deve encontrar no código gerado. Se a evidência não existe, a regra não foi seguida.
...
| Sem `any` | Nenhuma ocorrência de `: any`/`<any>`; body tipado como `unknown` antes do `safeParse`. |
| ESM + import type + `.js` | `import type { ... }` para tipos; imports relativos terminam em `.js` (`"../../shared/logger.js"`); sem `require`/`export default`. |
| Validação de input | `const parsed = <nome>RequestSchema.safeParse(...)` + `if (!parsed.success) throw new ValidationError(...)`. |
| Validação de output | `const validated = <nome>ResponseSchema.safeParse(response)` + `if (!validated.success) throw new InternalServerError(...)`. |
| Schemas `.strict()` | Todo `z.object({...})` encadeia `.strict()`. |
| Context budget (ADR-0002) | No schema de input: `top_k: z.number().int().min(1).max(5).optional()` e `conversation_history: z.array(...).max(3).optional()`. |
| `source_document` obrigatório | Objeto de resposta contém `source_document: ...` **e** `responseSchema` declara `source_document` como campo não-opcional. |
| `vigencia` (ADR-0003) | `source_document` (tipo e schema) inclui `vigencia: z.string()...optional()`. |
| Logging pino | `import { logger } from "../../shared/logger.js"`; `logger.info({ invocationId, method, url }, "...")` no início; zero `console.*`. |
| Log de erro com `{ err }` | `logger.error({ err: error instanceof Error ? error : undefined, invocationId }, "...")` no `catch`. |
...
| Config via `config.ts` | Variáveis lidas por import de `../../shared/config.js`; nenhum `process.env.` em `handler.ts`/services. |
...
**F4 — Log fora do pipeline**
```typescript
console.error("falhou", err);                                // ❌
logger.error({ err }, "query handler failed");               // ✅
```
...
Use este bloco no Copilot Chat (com o repositório aberto) para validar a skill end-to-end:

```text
Leia e siga ESTRITAMENTE a skill skills/domain/azure-functions-endpoint.md deste repositório
(e as dependências que ela lista: AGENTS.md + skills/foundation/*). Em seguida, gere um endpoint
Azure Functions v4 em TypeScript para POST /api/query, em src/functions/query/.

Requisitos:
- Input: { question: string (não vazio), top_k?: number, conversation_history?: [...] }.
- Validação Zod de INPUT e OUTPUT, com .strict(); enforce context budget (top_k máx 5, histórico máx 3).
- Logging estruturado com pino (logger de src/shared/logger.ts); zero console.*.
- Erros padronizados: try/catch + mapUnknownError + custom errors de src/shared/errors.ts; corpo { error, code }.
- Resposta SEMPRE com source_document (enforçado no schema de output).
- I/O e regra de negócio em src/services/, não no handler. Imports relativos com extensão .js. Sem any.

Entregue:
1) os arquivos handler.ts, validator.ts e response-builder.ts em src/functions/query/;
2) uma tabela curta "Regra da skill -> trecho do código" usando a seção §10 da skill como referência.
